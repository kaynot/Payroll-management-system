  ...
  return (
    <div className=" flex bg-background">
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            {navigation.map((item) => {
              const isActive = location.pathname.startsWith(item.href); // ✅ improved logic
              return (
                <Button
                  key={item.name}
      ...
      <div className="flex-1 flex flex-col lg:pl-64">
        {/* Top bar */}
        <header className="sticky top-0 z-30 h-16 border-b border-border backdrop-blur-sm bg-card/95 flex items-center justify-between px-4">
          <Button
            variant="ghost"
            size="icon"
            className="lg:hidden"
